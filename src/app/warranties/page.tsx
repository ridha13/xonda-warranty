...
const getToken=()=>{const c=document.cookie.split(";").find(x=>x.trim().startsWith("token="));return c?c.split("=")[1]:"";};
const hdr={Authorization:"Bearer "+getToken()};
const BASE="http://localhost:4000/api";

useEffect(()=>{
fetch(`${BASE}/auth/me`,{headers:hdr}).then(r=>r.json()).then(d=>setMe(d));
loadWarranties();
},[]);

const loadWarranties=()=>{
fetch(`${BASE}/warranties`,{headers:hdr}).then(r=>r.json()).then(d=>setWarranties(Array.isArray(d)?d:[]));
};
...
await fetch(`${BASE}/warranties/${id}`,{method:"PUT",headers:{...hdr,"Content-Type":"application/json"},body:JSON.stringify(body)});
setActionModal(null);
setReason("");
setMaxUsageCount("1");
loadWarranties();
